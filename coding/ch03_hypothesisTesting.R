...
# Loop through the columns starting from the 2nd column as the first one is for x-axis values and pause for each dataset
for (i in 2:11) {
  # Set up the plot with the first column as x and the i-th column as y
  plot(table2_2[[1]], table2_2[[i]], 
       type = 'p', 
       main = paste("Plot of Column", i), 
       xlab = "X-axis (Column 1)", 
       ylab = paste("Y-axis (Column", i, ")"))
  
  # You can add more customization here depending on your needs
  
  # Pause between plots - this line is optional, it's useful if you're running this interactively
  if (interactive()) {
...
# 3.3
## Let us calculate a 95% confidence interval for the coefficient on income  in the food expenditure model. library(PoEdata)
data("food")
?food
food
